// Safe Pagination - 15 ta per page + Cache
(function() {
  const PER_PAGE = 15;
  let currentPage = 1;
  function getProducts() { return window.allProducts || []; }
  ...
  function renderPagination(totalItems) {
    ...
    html += '<button onclick="window._pgChange(' + (currentPage-1) + ')" ' + (currentPage===1?'disabled':'') + ' style="padding:10px 16px;border:2px solid #10b981;border-radius:10px;cursor:pointer;">আগের</button>';
    ...
    html += '<button onclick="window._pgChange(' + (currentPage+1) + ')" ' + (currentPage===totalPages?'disabled':'') + ' style="padding:10px 16px;border:2px solid #10b981;border-radius:10px;cursor:pointer;">পরের</button>';
  ...
  let checkInterval = setInterval(function() {
    if (window.allProducts && window.allProducts.length > 0) {
      ...
      displayPage(1);
      try {
        localStorage.setItem('sk_all_products_v2', JSON.stringify(window.allProducts));
  ...
  if (searchInput) {
    searchInput.addEventListener('input', function() {
      setTimeout(function() { currentPage = 1; displayPage(1); }, 300);
    });
  }
})();
